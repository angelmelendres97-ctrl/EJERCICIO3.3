...
class EditOrdenCompra extends EditRecord
{
    ...
    public function onPedidosSeleccionados($pedidos, $connectionId, $motivo)
    {
        ...
        $pedidosSeleccionadosRaw = $this->parsePedidosRaw($pedidos);
        ...
        $detalles = DB::connection($connectionName)
            ->table('saedped')
            ->whereIn('dped_cod_pedi', $pedidosSeleccionadosRaw)
    ...
    private function resolveDetallePedidoData(array $data): array
    {
        ...
        $pares = collect($detalles)
            ...
            ->map(fn($detalle) => [
                'pedido_codigo' => (string) $detalle['pedido_codigo'],
                'pedido_detalle_id' => (string) $detalle['pedido_detalle_id'],
            ])
            ->unique()
            ->values();
        ...
        $pedidoCodigos = $this->normalizePedidoValues($pares->pluck('pedido_codigo')->unique()->values()->all());
        $detalleIds = $this->normalizePedidoValues($pares->pluck('pedido_detalle_id')->unique()->values()->all());
        ...
        foreach ($rows as $row) {
            $pedidoCodigo = (string) $row->dped_cod_pedi;
            $pedidoDetalle = (string) $row->dped_cod_dped;
            $detalle = trim((string) ($row->dped_det_dped ?? ''));
            $detalle = $detalle !== '' ? $detalle : null;
            $key = $this->buildPedidoKey($pedidoCodigo, $pedidoDetalle);
            $resultado[$key] = $detalle;

            $keyNormalizado = $this->buildPedidoKey(
                $this->normalizePedidoValue($pedidoCodigo),
                $this->normalizePedidoValue($pedidoDetalle)
            );

            $resultado[$keyNormalizado] = $detalle;
        }
    ...
    private function detallePedidoKey(array $detalle): ?string
    {
        ...
        return $this->buildPedidoKey((string) $pedidoCodigo, (string) $pedidoDetalleId);
    }

    private function buildPedidoKey(string $pedidoCodigo, string $pedidoDetalleId): string
    {
        return sprintf('%s:%s', $pedidoCodigo, $pedidoDetalleId);
    }

    private function normalizePedidoValue(string $value): string
    {
    ...
    private function normalizePedidoValues(array $values): array
    {
        $normalized = [];
        ...
        foreach ($values as $value) {
            $value = (string) $value;
            $normalized[] = $value;
            $normalized[] = $this->normalizePedidoValue($value);
        }

        return array_values(array_unique($normalized));
    }

    private function parsePedidosRaw(array|string|null $pedidos): array
    {
        if (empty($pedidos)) {
            return [];
        }

        $lista = is_array($pedidos)
            ? $pedidos
            : preg_split('/\\s*,\\s*/', trim((string) $pedidos));

        return collect($lista)
            ->filter(fn($pedido) => $pedido !== null && $pedido !== '')
            ->map(fn($pedido) => trim((string) $pedido))
            ->values()
            ->all();
    }
